...
`MAP_STAGES` の `gridRadius` まで区画が広がると次の段階へ引く。
段階が上がる瞬間に1マスが一気に縮み、画面に余白ができる。
**連続でジワジワ縮めない。** 自分が広げている感覚が薄れるので、節目で1回だけガクッと引く。

半径 r の段階を埋めきるのに必要な区画数は (2r+1)^2（順に 169 / 729 / 2809 / 8281）。
`resolveStageIndex` は着工中の区画も数に入れる（次の1つでリングが増えるなら先に引く）。

段階ごとの地形シルエット（皇居・湾岸線・日本列島・世界地図）と到達時の演出は**未実装**。
今はどの段階も道路グリッドのみ。しきい値もそれと合わせて調整する。
